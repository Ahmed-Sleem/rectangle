...
const viewer: UserPrincipal = { tenantId, userId: memberUserId, roles: ["none"], permissions: [] };
const outsider: UserPrincipal = { tenantId, userId: outsiderUserId, roles: ["none"], permissions: [] };
...
describe("ProjectTeamService", () => {
  ...
  it("lets a tenant project manager add a member and records an audit event", async () => {
    ...
    expect(audit.events[0]?.metadata).toMatchObject({ projectId, memberUserId, role: "manager" });
  });
  ...
  it("lets a project admin manage the team without tenant-wide rights", async () => {
    await service.addMember(admin, projectId, { userId: memberUserId, role: "owner" });
  ...
  it("allows demotion once another owner exists", async () => {
    await service.addMember(admin, projectId, { userId: memberUserId, role: "owner" });
    await service.addMember(admin, projectId, { userId: outsiderUserId, role: "owner" });
  ...
  it("rejects a role that is not one of the four", async () => {
    // `project_admin` is the name migration 019 retired. Accepting it would
    // write a row the database constraint then refuses, surfacing as a 500.
    await expect(
      service.addMember(admin, projectId, { userId: memberUserId, role: "project_admin" }),
...
describe("the capabilities reported for one project", () => {
  ...
  it("gives a project manager the actions their project role grants, with no company permission", async () => {
     ...
     */
    await service.addMember(admin, projectId, { userId: memberUserId, role: "manager" });
