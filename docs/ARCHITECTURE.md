...
- **Left menu:** shell-owned; generated from enabled `FeatureModule` entries so each instance can expose a different set/order of pages. The menu collapse control is a circular lower-left chrome control above Profile/Logout because it changes menu chrome, not the active page.
